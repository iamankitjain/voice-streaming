...
class ServiceDeskKnowledgeBaseClient {
  ...
  async retrieveFromKnowledgeBase(options: RetrieveOptions): Promise<Object> {
    ...
    try {
      ...
      if (
        ...
        response.retrievalResults.length === 0
      ) {
        return [];
      }
      ...
      for (const result of response.retrievalResults) {
        ...
        if (result.location?.s3Location) {
          source =
            result.location.s3Location.uri?.split("/").pop() ||
            "Unknown S3 file";
          location = result.location.s3Location.uri;
        } else if (result.location?.confluenceLocation) {
          source =
            result.location.confluenceLocation.url || "Unknown Confluence page";
...
export { ServiceDeskKnowledgeBaseClient, RetrieveOptions, RetrievalResult };
