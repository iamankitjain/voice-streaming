...
const tableName = 'service-desk-ticket';

export const queryData = async (ticketNumber: string): Promise<Object> => {
  const paramsIns  = {
    TableName: tableName,
...
export const createTicket = async (ticketNumber: string, empId: string, ticketType: string, issue: string, status: string): Promise<Object> => {
  const paramsIns = {
    TableName: tableName,
    Item: {
      ticketNumber: { S: ticketNumber },
