...
export class NovaSonicBidirectionalStreamClient {
  ...
  private async processToolUse(
    ...
    console.log("TOOL USER CONTENT", JSON.stringify(toolUseContent));
    switch (tool) {
      ...
      case "getmonopoly":
        try {
          const params = {
            input: {
              text: "how to build a house?",
            },
            retrieveAndGenerateConfiguration: {
              type: RetrieveAndGenerateType.KNOWLEDGE_BASE,
              knowledgeBaseConfiguration: {
                knowledgeBaseId: "KQTR3789HR", // replace with your KB ID
                modelArn:
                  "arn:aws:bedrock:us-east-1::foundation-model/anthropic.claude-v2",
              },
            },
          };
          const clientKb = new BedrockAgentRuntimeClient({
            region: "us-east-1",
          });
          const command = new RetrieveAndGenerateCommand(params);
          const response = await clientKb.send(command);
          if (response.output) {
          ...
          toolConfiguration: {
            tools: [
              ...
              },
              {
                toolSpec: {
                  name: "getmonopoly",
                  description: "Get information for anything about Monopoly.",
                  inputSchema: {
                    json: DefaultRagSchema,
                  },
                },
              },
              {
