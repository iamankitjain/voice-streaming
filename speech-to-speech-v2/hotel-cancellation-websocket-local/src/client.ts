  ...
  private async processToolUse(toolName: string, toolUseContent: object): Promise<Object> {
    ...
    switch (tool) {
      ...
      case "cancelreservationtool":
        ...
        return result;
      case "callbackrequesttool":
        const result2 = await handleToolCall(toolUseContent);
        console.log(`Tool ${tool} result: ${JSON.stringify(result2)}`);
        return result2;
      default:
        ...
        promptStart: {
          ...
          toolConfiguration: {
            tools: [
              ...
              {
                toolSpec: {
                  name: "callbackRequestTool",
                  description: "Submit callback request for Customer",
                  inputSchema: {
                    json: CallbackRequestToolSchema
                  }
                }
              }
            ]
