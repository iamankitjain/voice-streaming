...
export const callbackRequest = async (params: CallbackRequestParams): Promise<CallbackRequestResult> => {
    console.log(`Submit callback request for customer to talk to live agent ${params.phoneNumber}`);
    return {
        success: true,
...
export const processToolCalls = async (toolName: string, toolInput: any): Promise<any> => {
    switch (toolName) {
        ...
        case 'callbackRequestTool':
            return await callbackRequest(toolInput);
